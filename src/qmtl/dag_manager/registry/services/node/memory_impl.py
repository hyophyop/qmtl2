...
from .validation import validate_proto_node


class InMemoryNodeService(NodeManagementService):
    def __init__(self):
        self._nodes: Dict[str, qmtl_datanode_pb2.DataNode] = {}
        self._lock = Lock()
        ...
        # 노드 상태 저장용 Dictionary (str 또는 protobuf status 사용)
        self._node_statuses: Dict[str, str] = {}

    def create_node(self, node: qmtl_datanode_pb2.DataNode) -> str:
        """
    ...
    def get_node(self, node_id: str) -> Optional[qmtl_datanode_pb2.DataNode]:
        """Get a node by ID."""
    ...
    def list_nodes(self) -> List[qmtl_datanode_pb2.DataNode]:
        """List all nodes."""
        return list(self._nodes.values())

    def list_zero_deps(self) -> List[qmtl_datanode_pb2.DataNode]:
    ...
    def list_by_tags(
        ...
        match_mode: str = "AND",
    ) -> List[qmtl_datanode_pb2.DataNode]:
        """
    ...
    def validate_node(self, node: qmtl_datanode_pb2.DataNode) -> None:
        """Validate DataNode (protobuf). Will raise ValidationError on failure."""
        validate_proto_node(node)
    ...
    def get_strategy_nodes(self, strategy_version_id: str) -> List[qmtl_datanode_pb2.DataNode]:
        """
    ...
    def update_node_status(self, node_id: str, status):
        """해당 DataNode의 상태/메타데이터를 저장/갱신한다."""
        with self._lock:
            self._node_statuses[node_id] = status

    def get_node_dependencies(self, node_id: str) -> List[str]:
        """해당 노드가 의존하는 노드 ID 목록 반환"""
        node = self._nodes.get(node_id)
        return list(node.dependencies) if node else []

    def add_dependency(self, node_id: str, dependency_id: str) -> None:
        """노드 간 DEPENDS_ON 관계 생성"""
        with self._lock:
            node = self._nodes.get(node_id)
            dep_node = self._nodes.get(dependency_id)
            if node is None or dep_node is None:
                return  # 존재하지 않는 노드 무시
    ...
    def remove_dependency(self, node_id: str, dependency_id: str) -> None:
        """노드 간 DEPENDS_ON 관계 삭제"""
        with self._lock:
            node = self._nodes.get(node_id)
            if node and dependency_id in node.dependencies:
