...
from qmtl.models.generated.qmtl_status_pb2 import NodeStatus, NodeErrorDetail, PipelineStatus

# NodeErrorDetail, PipelineStatus도 protobuf에서 import 필요시 추가
...
def test_pipeline_status_error_detail():
    now = datetime.utcnow()
    err = NodeErrorDetail(code="P2001", message="Pipeline error", occurred_at=now)
...
def test_node_fault_error_exception():
    with pytest.raises(NodeFaultError) as exc:
        raise NodeFaultError(
            node_id="abcd1234abcd1234abcd1234abcd1234", message="Node crashed", code="E9999"
        )
...
def test_data_consistency_error_exception():
    with pytest.raises(DataConsistencyError) as exc:
        raise DataConsistencyError(
            node_id="abcd1234abcd1234abcd1234abcd1234", message="Duplicate execution detected"
        )
