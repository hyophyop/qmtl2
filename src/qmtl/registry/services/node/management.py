...
class Neo4jNodeManagementService(NodeManagementService):
    # MULTI-4: 파이프라인 내 실행 가능한(의존성 모두 충족, 대기/ready 상태) 노드 목록 반환
    def get_ready_nodes(self, pipeline_id: str, nodes, edges) -> List[DataNode]:
        """
        파이프라인 내 실행 가능한(의존성 모두 충족, 대기/ready 상태) 노드 목록 반환
        nodes: List[DataNode], edges: List[dict]
        1. edges로부터 의존성 그래프 구성
        2. 각 노드의 NodeStatus가 'READY' 또는 None이고, 모든 upstream이 완료된 노드만 반환
        (간단화: 의존성 없는 노드만 반환)
        """
        node_ids = {n.node_id for n in nodes}
        dep_map = {n.node_id: set() for n in nodes}
        for edge in edges:
            dep_map[edge['target']].add(edge['source'])
        ready = []
        for n in nodes:
            # 모든 의존성이 node_ids에 포함되어 있고, NodeStatus가 READY 또는 None
            if not dep_map[n.node_id]:
                # 실제로는 NodeStatus 조회 필요
                ready.append(n)
        return ready
    ...
    def create_node(self, node: DataNode) -> str:
        ...
        interval_settings_dict = (
            json.dumps(node.interval_settings.model_dump() if node.interval_settings else None)
        )
        params = {
    ...
    def add_dependency(self, node_id: str, dependency_id: str) -> None:
        ...
        self.neo4j_client.execute_query(query, {"node_id": node_id, "dependency_id": dependency_id}, self.database)
