...
from qmtl.sdk.models import IntervalEnum, IntervalSettings
...
class TestMetadataService:
    ...
    def test_create_and_get_node(self):
        # stream_settings 필수
        intervals = {IntervalEnum.HOUR: IntervalSettings(interval=IntervalEnum.HOUR, period=1).model_dump()}
        stream_settings = NodeStreamSettings(intervals=intervals)
        # node_id는 32자리 소문자 hex
        node = DataNode(node_id="a"*32, type="FEATURE", data_format={"type": "json"}, params={}, dependencies=[], ttl=3600, stream_settings=stream_settings)
