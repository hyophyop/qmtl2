...
class MockStrategySnapshotService:
    def get_snapshots(self, pipeline_id):
        # Always return a dummy snapshot for test
        class DummySnapshot:
            nodes = ["dummy-node-id"]
            edges = []
            metadata = {}
...
from qmtl.registry.api import get_node_service, get_strategy_service, get_snapshot_service
app.dependency_overrides[get_node_service] = lambda: MockNeo4jNodeManagementService()
app.dependency_overrides[get_strategy_service] = lambda: MockNeo4jStrategyManagementService()
app.dependency_overrides[get_snapshot_service] = lambda: MockStrategySnapshotService()
